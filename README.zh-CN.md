...
一个强大的 [Teable](https://teable.ai) 插件，用于从表格数据生成条形码和二维码并保存为可下载的附件。
...
- 🎨 **可自定义外观** - 控制尺寸、前景色/背景色
- 📐 **纠错级别** - 从 L (~7%)、M (~15%)、Q (~25%)、H (~30%) 中选择，推荐使用 H
- 📁 **多种格式** - 支持 PNG 和 SVG 输出格式
- 👁️ **实时预览** - 实时预览二维码外观

### 通用功能
- 🎯 **字段映射** - 灵活选择源数据字段和目标附件字段
- 📈 **进度跟踪** - 美观的卡片式UI显示实时生成进度和统计信息
...
### 条形码生成
1. **切换到条形码标签页** - 选择"条形码"标签页
2. **选择视图** - 选择包含要处理记录的视图
3. **选择数据源字段** - 选择包含条形码生成数据的字段（文本或数字字段）
4. **选择附件字段** - 选择用于存储生成条形码的附件字段
5. **配置条形码** - 自定义条形码格式、外观和文本设置
6. **预览** - 实时预览条形码外观
7. **生成** - 点击"生成条形码"按钮处理所有记录
8. **查看结果** - 生成的条形码以图片形式保存在附件字段中

### 二维码生成
1. **切换到二维码标签页** - 选择"二维码"标签页
2. **选择视图** - 选择包含要处理记录的视图
3. **选择数据源字段** - 选择包含二维码生成数据的字段（文本或数字字段）
4. **选择附件字段** - 选择用于存储生成二维码的附件字段
5. **配置二维码** - 自定义纠错级别、尺寸、颜色和输出格式
6. **预览** - 实时预览二维码外观
7. **生成** - 点击"生成二维码"按钮处理所有记录
...
├── components/          # React组件
│   ├── BarcodeGenerator.tsx  # 主条形码/二维码生成器组件
